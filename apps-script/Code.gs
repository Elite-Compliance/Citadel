...
const CITADEL_ACCESS_MODULES = ['command-center', 'region-health', 'data-connections', 'inbox', 'tasks', 'legal', 'reviews', 'pricing', 'labor-pricing', 'templates', 'orders', 'crew-pay', 'fleet', 'contractors', 'registrations', 'liens', 'suppliers', 'collections'];
...
const SHEETS = {
  ...
  crewInvoiceLines: 'CrewInvoiceLines',
  crewRateAnalysis: 'CrewRateAnalysis',
  crewInvoiceExceptions: 'CrewInvoiceExceptions',
  crewInvoiceImportLog: 'CrewInvoiceImportLog',
...
const CREW_INVOICE_HEADERS = ['invoice_id', 'invoice_number', 'job_id', 'job_number', 'job_url', 'customer', 'region', 'crew_name', 'trade', 'invoice_status', 'invoice_date', 'total_amount', 'approved_by', 'approved_on', 'paid_on', 'archived_status', 'ordered_labor_total', 'invoice_labor_total', 'master_labor_total', 'order_variance', 'master_variance', 'comparison_status', 'source_url', 'source_updated_at', 'import_batch_id', 'active'];
const CREW_INVOICE_LINE_HEADERS = ['line_id', 'invoice_id', 'job_id', 'order_id', 'order_line_id', 'item_name', 'normalized_item', 'uom', 'quantity', 'original_quantity', 'invoice_unit_price', 'invoice_total', 'custom_labor', 'note', 'requirements', 'order_unit_price', 'order_quantity', 'order_total', 'master_unit_price', 'master_total', 'invoice_vs_order_variance', 'invoice_vs_master_variance', 'price_source', 'match_method', 'order_match_confidence', 'match_confidence', 'comparison_status', 'imported_at', 'active'];
const CREW_RATE_ANALYSIS_HEADERS = ['analysis_id', 'region', 'crew_name', 'trade', 'item_name', 'uom', 'invoice_count', 'job_count', 'common_rate', 'minimum_rate', 'maximum_rate', 'rate_count', 'inconsistent'];
const CREW_INVOICE_EXCEPTION_HEADERS = ['exception_id', 'invoice_id', 'line_id', 'exception_type', 'status', 'reason', 'reviewed_by', 'reviewed_at', 'created_at', 'active'];
const CREW_INVOICE_IMPORT_LOG_HEADERS = ['import_id', 'started_at', 'completed_at', 'source', 'regions_expected', 'regions_completed', 'invoices_discovered', 'invoices_retained', 'invoice_lines_retained', 'custom_labor_lines', 'order_variances', 'master_variances', 'unmatched_lines', 'status', 'warnings', 'error'];
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'getCrewPay') {
      return output_(e, { ok: true, data: getCrewPay(), version: CITADEL_VERSION });
    }

    if (action === 'setupCrewPaySheet') {
      return output_(e, { ok: true, data: setupCrewPaySheet(), version: CITADEL_VERSION });
    }
...
function citadelAccessRule_(action) {
  const rules = {
    ...
    getTemplatePriceOverrides: ['templates', 'view'], saveTemplatePriceOverride: ['templates', 'edit', true], getOrders: ['orders', 'view'], getCrewPay: ['crew-pay', 'view'],
...
function scopeCitadelResponse_(action, data, context) {
  ...
  if (action === 'getCrewPay') {
    scoped.invoices = (data.invoices || []).filter(function(row) { return citadelRowInRegionScope_(row, user); });
    const ids = scoped.invoices.reduce(function(map, row) { map[String(row.invoice_id || '')] = true; return map; }, {});
    scoped.lines = citadelFilterRelated_(data.lines, ['invoice_id'], ids);
    scoped.exceptions = citadelFilterRelated_(data.exceptions, ['invoice_id'], ids);
    scoped.analysis = (data.analysis || []).filter(function(row) { return citadelRowInRegionScope_(row, user); });
    scoped.metrics = buildCrewPayMetrics_(scoped.invoices, scoped.lines, scoped.analysis);
    return scoped;
  }
...
function setupCrewPaySheet() {
  const spreadsheetId = SPREADSHEETS.payments;
  if (!spreadsheetId) throw new Error('The protected operations spreadsheet is not configured.');
  ensureSheetWithHeaders_(spreadsheetId, SHEETS.crewInvoices, CREW_INVOICE_HEADERS);
  ensureSheetWithHeaders_(spreadsheetId, SHEETS.crewInvoiceLines, CREW_INVOICE_LINE_HEADERS);
  ensureSheetWithHeaders_(spreadsheetId, SHEETS.crewRateAnalysis, CREW_RATE_ANALYSIS_HEADERS);
  ensureSheetWithHeaders_(spreadsheetId, SHEETS.crewInvoiceExceptions, CREW_INVOICE_EXCEPTION_HEADERS);
  ensureSheetWithHeaders_(spreadsheetId, SHEETS.crewInvoiceImportLog, CREW_INVOICE_IMPORT_LOG_HEADERS);
  return {
    spreadsheet_id: spreadsheetId,
    sheets: [SHEETS.crewInvoices, SHEETS.crewInvoiceLines, SHEETS.crewRateAnalysis, SHEETS.crewInvoiceExceptions, SHEETS.crewInvoiceImportLog],
    status: 'Ready'
  };
}
...
function getCrewPay() {
  setupCrewPaySheet();
  const spreadsheetId = SPREADSHEETS.payments;
  const invoices = readSheetObjects_(spreadsheetId, SHEETS.crewInvoices).filter(isActiveRow_);
  const lines = readSheetObjects_(spreadsheetId, SHEETS.crewInvoiceLines).filter(isActiveRow_);
...
function buildCrewPayMetrics_(invoices, lines, analysis) {
  const source = invoices || [];
  const lineSource = lines || [];
  return {
    total_invoices: source.length,
    pending_approval: source.filter(function(row) { return /pending/i.test(String(row.invoice_status || '')); }).length,
    rate_variances: lineSource.filter(function(row) { return /variance/i.test(String(row.comparison_status || '')); }).length,
    custom_labor: lineSource.filter(function(row) { return /custom/i.test(String(row.comparison_status || '')); }).length,
    needs_review: source.filter(function(row) { return /review|missing/i.test(String(row.comparison_status || '')); }).length,
    inconsistent_rates: (analysis || []).filter(function(row) { return /yes|true|1/i.test(String(row.inconsistent || '')); }).length
  };
}
