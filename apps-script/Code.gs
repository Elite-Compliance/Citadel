...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'changePassword') {
      CITADEL_REQUEST_CONTEXT = authorizeCitadelRequest_(e, action);
      return citadelLoginBridgeOutput_(changeCitadelPassword_(payload, CITADEL_REQUEST_CONTEXT));
    }
    ...
    if (action === 'loginWithPassword' || action === 'changePassword') {
...
function getCitadelAuthConfig_() {
  ...
  const challenge = enabled ? createCitadelLoginChallenge_() : '';
  return {
...
function loginWithPassword_(payload) {
  ...
  if (!verifyCitadelPassword_(password, user)) {
...
function saveCitadelUser_(payload, actor) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error('Enter a valid Google account email address.');
  ...
  const record = {
    ...
    google_subject: existing.google_subject || '',
    email: email,
  ...
  appendAccessAudit_({
    ...
    email: actor.email,
    action: existing.user_id ? 'update_user' : 'invite_user',
    resource: email,
    outcome: 'allowed',
...
function getContractors() {
  ...
  const crews = sheetExists_(spreadsheetId, SHEETS.contractorCrews) ? readSheetObjects_(spreadsheetId, SHEETS.contractorCrews).filter(isActiveRow_) : [];
