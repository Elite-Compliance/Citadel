...
function handleCitadelAuthMessage(event){if(!event.data||event.data.type!=='citadel-auth'||!citadelAuth.loginFrame||!citadelAuth.pendingRequest)return;var trusted=false;try{var host=new URL(event.origin).hostname;trusted=event.origin==='https://script.google.com'||host==='script.googleusercontent.com'||/\.script\.googleusercontent\.com$/.test(host)}catch(error){}if(!trusted)return;var payload=event.data.payload||{};var pending=citadelAuth.pendingRequest;clearCitadelFormRequest();pending.resolve(payload)}
