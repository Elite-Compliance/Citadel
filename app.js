...
var citadelAuth={config:null,user:null,sessionToken:"",csrfToken:"",expiresAt:"",loginFrame:null};
...
function waitForGoogleIdentity(){return new Promise(function(resolve,reject){var attempts=0;function check(){if(window.google&&google.accounts&&google.accounts.id){resolve();return}attempts+=1;if(attempts>100){reject(new Error('Google sign-in did not load.'));return}window.setTimeout(check,100)}check()})}
function renderCitadelLogin(message){document.body.classList.add('citadel-auth-required');var existing=document.querySelector('[data-citadel-login]');if(existing)existing.remove();var overlay=document.createElement('div');overlay.className='citadel-login-overlay';overlay.setAttribute('data-citadel-login','');overlay.innerHTML='<main class="citadel-login-card" aria-labelledby="citadelLoginTitle"><div class="citadel-login-mark" aria-hidden="true">C</div><p>Operations Platform</p><h1 id="citadelLoginTitle">Sign in to Citadel</h1><span>Use an approved Google account. Access is controlled by the Citadel administrator.</span><div class="citadel-login-message" role="alert">'+escapeHtml(message||'')+'</div><div data-google-signin></div><small>Need access? Contact your Citadel administrator.</small></main>';document.body.appendChild(overlay);if(!citadelAuth.config||!citadelAuth.config.google_client_id){overlay.querySelector('.citadel-login-message').textContent='Google sign-in is not configured yet.';return}waitForGoogleIdentity().then(function(){google.accounts.id.initialize({client_id:citadelAuth.config.google_client_id,callback:submitCitadelGoogleCredential,nonce:citadelAuth.config.login_challenge,ux_mode:'popup',auto_select:false});google.accounts.id.renderButton(overlay.querySelector('[data-google-signin]'),{type:'standard',theme:'outline',size:'large',text:'signin_with',shape:'rectangular',width:280})}).catch(function(error){overlay.querySelector('.citadel-login-message').textContent=error.message})}
function submitCitadelGoogleCredential(response){if(!response||!response.credential){renderCitadelLogin('Google did not return a sign-in credential.');return}var frame=document.createElement('iframe');var frameName='citadel-auth-'+Date.now();frame.name=frameName;frame.hidden=true;document.body.appendChild(frame);citadelAuth.loginFrame=frame;var form=document.createElement('form');form.method='post';form.action=CITADEL_API_URL;form.target=frameName;form.hidden=true;[['action','loginWithGoogle'],['credential',response.credential],['login_challenge',citadelAuth.config.login_challenge]].forEach(function(entry){var input=document.createElement('input');input.type='hidden';input.name=entry[0];input.value=entry[1];form.appendChild(input)});document.body.appendChild(form);form.submit();form.remove()}
function handleCitadelAuthMessage(event){if(!event.data||event.data.type!=='citadel-auth'||!citadelAuth.loginFrame||event.source!==citadelAuth.loginFrame.contentWindow)return;var trusted=false;try{var host=new URL(event.origin).hostname;trusted=event.origin==='https://script.google.com'||host==='script.googleusercontent.com'||/\.script\.googleusercontent\.com$/.test(host)}catch(error){}if(!trusted)return;var payload=event.data.payload||{};citadelAuth.loginFrame.remove();citadelAuth.loginFrame=null;if(!payload.ok){clearCitadelAuthSession();loadCitadelAuthConfig().then(function(){renderCitadelLogin(payload.error||'Sign-in failed.')});return}saveCitadelAuthSession(payload);citadelAuth.user=payload.user||null;document.body.classList.remove('citadel-auth-required');var overlay=document.querySelector('[data-citadel-login]');if(overlay)overlay.remove();syncCitadelAccountMenu();bootCitadelApp()}
...
function renderCitadelUsersModal(users){var modal=document.createElement('div');modal.className='citadel-modal-backdrop';modal.innerHTML='<section class="workspace-entry-modal citadel-users-modal" role="dialog" aria-modal="true" aria-label="User Access"><div class="modal-head"><div><h3>User Access</h3><p>Add Google accounts and control their modules, regions, and actions.</p></div><button type="button" data-close-modal aria-label="Close">x</button></div><div class="citadel-users-layout"><div class="citadel-user-list"><button type="button" data-citadel-user-index="new"><strong>+ Add user</strong><span>Invite an approved Google account</span></button>'+users.map(function(user,index){return '<button type="button" data-citadel-user-index="'+index+'"><strong>'+escapeHtml(user.display_name||user.email)+'</strong><span>'+escapeHtml(user.role+' · '+user.status)+'</span><small>'+escapeHtml(user.email)+'</small></button>'}).join('')+'</div><div data-citadel-user-form></div></div></section>';function show(user){user=user||{role:'Read Only',status:'Invited',modules:'',permissions:'view',regions:''};var host=modal.querySelector('[data-citadel-user-form]');host.innerHTML='<form class="citadel-user-form"><label>Email *<input name="email" type="email" required value="'+escapeHtml(user.email||'')+'" '+(user.email?'readonly':'')+'></label><label>Display name<input name="display_name" value="'+escapeHtml(user.display_name||'')+'"></label><div class="citadel-user-form-row"><label>Role<select name="role">'+['Admin','Manager','Editor','Read Only'].map(function(value){return '<option '+(value===user.role?'selected':'')+'>'+value+'</option>'}).join('')+'</select></label><label>Status<select name="status">'+['Invited','Active','Suspended','Deactivated'].map(function(value){return '<option '+(value===user.status?'selected':'')+'>'+value+'</option>'}).join('')+'</select></label></div><fieldset><legend>Modules they can view</legend><div class="citadel-access-checks">'+citadelModuleOptions(user.modules)+'</div></fieldset><label>Regions<input name="regions" placeholder="CLE, CHI, PIT — or * for all" value="'+escapeHtml(user.regions||'')+'"><small>Separate multiple regions with commas.</small></label><fieldset><legend>Actions allowed</legend><div class="citadel-access-checks">'+citadelPermissionOptions(user.permissions)+'</div></fieldset><div class="workspace-entry-actions"><button type="button" data-close-modal>Cancel</button><button type="submit" class="primary">Save user</button></div><p class="citadel-user-form-status" role="status"></p></form>';host.querySelector('form').addEventListener('submit',function(event){event.preventDefault();saveCitadelUserForm(event.currentTarget,modal)})}modal.addEventListener('click',function(event){var choice=event.target.closest('[data-citadel-user-index]');if(choice){var index=choice.getAttribute('data-citadel-user-index');show(index==='new'?null:users[Number(index)]);return}if(event.target===modal||event.target.closest('[data-close-modal]'))closeLiensReportsModal()});document.body.appendChild(modal);show(users[0]||null)}
function saveCitadelUserForm(form,modal){var payload={};new FormData(form).forEach(function(value,key){if(key==='modules'||key==='permissions'){if(!payload[key])payload[key]=[];payload[key].push(value)}else payload[key]=String(value).trim()});payload.modules=(payload.modules||[]).join(',');payload.permissions=(payload.permissions||[]).join(',');var status=form.querySelector('.citadel-user-form-status');status.textContent='Saving access...';var params=Object.keys(payload).map(function(key){return encodeURIComponent(key)+'='+encodeURIComponent(payload[key])}).join('&');jsonp(CITADEL_API_URL+'?action=saveCitadelUser&'+params).then(function(response){if(!response||!response.ok)throw new Error(response&&response.error?response.error:'Save failed');closeLiensReportsModal();openCitadelUsersModal()}).catch(function(error){status.textContent=error.message||'Save failed.'})}
...
var PRICING_CACHE_VERSION=2;
var PRICING_CACHE_MAX_AGE_MS=24*60*60*1000;
...
function pricingVariantPrices(variants){var prices=[];(variants||[]).forEach(function(variant){var raw=String(variant&&variant.price!==undefined&&variant.price!==null?variant.price:'').trim();if(!raw)return;var number=Number(raw.replace(/[^0-9.-]/g,''));if(!isFinite(number))return;if(!prices.some(function(price){return price.number===number}))prices.push({number:number,raw:raw})});return prices.sort(function(a,b){return a.number-b.number})}
function pricingPriceSummary(variants){var prices=pricingVariantPrices(variants);if(!prices.length)return 'Not provided';if(prices.length===1)return prices[0].raw;return pricingMoneyLabel(prices[0].number)+' – '+pricingMoneyLabel(prices[prices.length-1].number)}
function pricingGroupedRows(rows){var map={};var groups=[];rows.forEach(function(row){var key=pricingGroupKey(row);if(!map[key]){map[key]={key:key,base:Object.assign({},row),variants:[]};map[key].base._groupKey=key;groups.push(map[key])}map[key].variants.push(row)});groups.forEach(function(group){group.variants.sort(function(a,b){return String(pricingColorValue(a)||'').localeCompare(String(pricingColorValue(b)||''))});var colors=[];var seen={};group.variants.forEach(function(variant){var color=pricingColorValue(variant);if(color&&!seen[color]){seen[color]=true;colors.push(color)}});var prices=pricingVariantPrices(group.variants);var selected=pricingSelectedVariant(group);if(selected)Object.assign(group.base,selected);else if(colors.length>1){['color','item_final','item_number','item','color_code'].forEach(function(key){group.base[key]=''});group.base.price=pricingPriceSummary(group.variants);group.base._needsColor=true}else if(prices.length>1)group.base.price=pricingPriceSummary(group.variants);group.base._priceVaries=prices.length>1;group.base._groupKey=group.key;group.base._variants=group.variants});return groups.map(function(group){return group.base})}
...
function pricingMoneyLabel(value){var text=String(value===undefined||value===null?'':value).trim();if(!text)return '';var cleaned=text.replace(/[^0-9.-]/g,'');if(!cleaned)return text;var number=Number(cleaned);if(!isFinite(number))return text;return '$'+number.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}
function pricingCellHtml(row,key){if(key==='color'){var variants=row._variants||[];var colors=[];var seen={};variants.forEach(function(variant){var color=pricingColorValue(variant);if(color&&!seen[color]){seen[color]=true;colors.push(color)}});if(colors.length>1){if(!row._priceVaries)return '<span title="All available colors">All colors</span>';var current=pricingColorSelections[row._groupKey]||'';return '<span><select class="pricing-color-select" data-pricing-color-group="'+escapeHtml(row._groupKey||'')+'"><option value="">Select color</option>'+colors.map(function(color){return '<option value="'+escapeHtml(color)+'" '+(String(color)===String(current)?'selected':'')+'>'+escapeHtml(color)+'</option>'}).join('')+'</select></span>'}}var value=row[key];if(row._needsColor&&['item_final','item_number','item','color_code'].indexOf(key)>-1)value='';if(/price|rate|amount|cost|total/i.test(key)&&String(value===undefined||value===null?'':value).trim()!=='')value=pricingMoneyLabel(value);return '<span title="'+escapeHtml(value)+'">'+escapeHtml(value)+'</span>'}
...
function openPricingDb(){return new Promise(function(resolve,reject){if(!window.indexedDB){reject(new Error('Browser cache unavailable'));return}var req=indexedDB.open(citadelScopedCacheKey(PRICING_CACHE_DB),PRICING_CACHE_VERSION);req.onupgradeneeded=function(){var db=req.result;var tx=req.transaction;if(!db.objectStoreNames.contains(PRICING_CACHE_STORE))db.createObjectStore(PRICING_CACHE_STORE,{keyPath:'_row'});else tx.objectStore(PRICING_CACHE_STORE).clear();if(!db.objectStoreNames.contains('meta'))db.createObjectStore('meta',{keyPath:'key'});else tx.objectStore('meta').clear()};req.onsuccess=function(){resolve(req.result)};req.onerror=function(){reject(req.error)}})}
...
function hydratePricingFromCache(){return pricingDbGetAll().then(function(payload){if(!payload.rows.length)return false;var savedAt=Number(payload.meta&&payload.meta.savedAt||0);if(!savedAt||Date.now()-savedAt>PRICING_CACHE_MAX_AGE_MS)return false;pricingState.allRows=payload.rows;pricingState.headers=payload.meta.headers||[];pricingState.keys=payload.meta.keys||Object.keys(payload.rows[0]||{});pricingState.sheet=payload.meta.sheet||'Cached pricing';pricingState.cacheReady=true;pricingState.loading=false;pricingState.cacheMessage='Cached pricing ready';refreshPricingFacets();applyPricingLocalView();return true}).catch(function(){return false})}
...
function pricingGridTemplate(columns){var widths={supplier:'minmax(64px,.55fr)',state:'48px',brand:'54px',elite_product_name:'minmax(260px,2.6fr)',color:'minmax(145px,1.2fr)',item_final:'minmax(120px,1fr)',type:'minmax(105px,.9fr)',uom:'54px',price:'88px'};return columns.map(function(key){return widths[key]||'minmax(96px,1fr)'}).join(' ')}
function renderPricingTable(columns){if((pricingState.loading||pricingState.cacheLoading)&&!pricingState.rows.length)return '<div class="pricing-loading-card"><div class="loading-pulse"></div><h3>Loading pricing</h3><p>Finding the latest pricing rows.</p></div>';if(pricingState.error&&!pricingState.rows.length)return '<div class="pricing-empty">'+escapeHtml(pricingState.error)+'</div>';if(!pricingState.rows.length)return '<div class="pricing-empty">No pricing records found.</div>';var displayRows=pricingState.rows;var grid=pricingGridTemplate(columns);return '<div class="pricing-table"><div class="pricing-table-head" style="grid-template-columns:'+grid+'">'+columns.map(function(key){return '<span>'+escapeHtml(pricingColumnLabel(key))+'</span>'}).join('')+'</div>'+displayRows.map(function(row){return '<div class="pricing-row" style="grid-template-columns:'+grid+'">'+columns.map(function(key){return pricingCellHtml(row,key)}).join('')+'</div>'}).join('')+'</div>'}
