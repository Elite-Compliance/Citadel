...
var pages=[{id:"command-center",label:"Command Center"},{id:"region-health",label:"Region Health"},{id:"data-connections",label:"Data Connections"},{id:"inbox",label:"Inbox"},{id:"tasks",label:"Tasks"},{id:"legal",label:"Legal"},{id:"reviews",label:"Reviews"},{id:"pricing",label:"Material Pricing"},{id:"labor-pricing",label:"Labor Pricing"},{id:"templates",label:"Templates"},{id:"fleet",label:"Fleet",children:[{id:"fleet-vehicles",label:"Vehicles"},{id:"fleet-drivers",label:"Drivers"}]},{id:"contractors",label:"Contractors"},{id:"suppliers",label:"Suppliers"},{id:"registrations",label:"Registrations"},{id:"liens",label:"Liens"},{id:"collections",label:"Collections"}];
...
function initCitadelAuth(){readCitadelAuthSession();window.addEventListener('message',handleCitadelAuthMessage);return loadCitadelAuthConfig().then(function(config){if(!config.enforcement_enabled){document.body.classList.remove('citadel-auth-pending');return true}if(!citadelAuth.sessionToken){renderCitadelLogin();return false}return jsonp(CITADEL_API_URL+'?action=getCurrentUser').then(function(response){if(!response||!response.ok||!response.data)throw new Error('Session unavailable');citadelAuth.user=response.data;document.body.classList.remove('citadel-auth-pending');syncCitadelAccountMenu();return true}).catch(function(){clearCitadelAuthSession();renderCitadelLogin('Your session expired. Please sign in again.');return false})}).catch(function(error){renderCitadelLogin(error.message||'Citadel sign-in is temporarily unavailable.');return false})}
...
var TEMPLATE_PREVIEW_RECORD={
  ...
  materials:[
    templateItem('IKO DYNASTY CLASS 3','','SHINGLE Â· Coverage 1 SQ'),
    templateItem('IKO CAMBRIDGE','','SHINGLE Â· Coverage 1 SQ'),
    templateItem('IKO HIP & RIDGE 12','','HIP AND RIDGE SHINGLES Â· Coverage 30 LF'),
    templateItem('IKO 7" STARTER 123\' PER BUNDLE','BDL','ROOFING STARTER Â· Coverage 105 LF Â· Include rakes'),
    templateItem('ICE & WATER ALCO GRANULATED 2 SQ NOT BOX','ROLL','ICE AND WATER Â· Coverage 60 LF'),
    templateItem('VB DEFENDER SYNTHETIC UNDERLAYMENT 10 SQ','ROLL','FELT Â· Coverage 9 SQ Â· Pitch 2/12â€“99/12'),
    templateItem('T-DRIP 1.75" Q/E','','DRIP EDGES Â· Coverage 10 LF Â· Using gutter apron'),
    templateItem('Q/E GUTTER APRON','','GUTTER APRON Â· Coverage 10 LF'),
    templateItem('AIRHAWK VENT SLA SLANT','',''),
    templateItem("SHINGLEVENT II 4' BLACK",'PC','RIDGE VENTS Â· Coverage 4 LF'),
    ...
    templateItem('COIL NAIL 1-1/4"','EACH','NAILS Â· Coverage 15 SQ'),
    templateItem('PLASTIC CAP NAIL 1-1/4" 2M/CTN','BOX','CAP NAILS Â· Coverage 20 SQ'),
    templateItem('OSI QUAD SEALANT #','','ROOFING CAULK Â· Coverage 15 SQ'),
    templateItem('Q/E SPRAY PAINT','','ROOFING SPRAY PAINT Â· Coverage 20 EA'),
    templateItem('W-VALLEY 18"X10\'','','VALLEY METAL Â· Coverage 10 LF'),
    templateItem('QA C24 COIL .024 #','',''),
    templateItem('OSB 4X8 7/16','SHT'),
    templateItem('QUARRIX SMART PLUG PART 8" #99008','EACH'),
    templateItem('STEP FLASHING','','STEP FLASHING Â· Coverage 55 LF'),
...
function pricingCellHtml(row,key){if(key==='color'){var variants=row._variants||[];var colors=[];var seen={};variants.forEach(function(variant){var color=pricingColorValue(variant);if(color&&!seen[color]){seen[color]=true;colors.push(color)}});if(colors.length>1){var current=pricingColorSelections[row._groupKey]||'';return '<span><select class="pricing-color-select" data-pricing-color-group="'+escapeHtml(row._groupKey||'')+'"><option value="">Select color</option>'+colors.map(function(color){return '<option value="'+escapeHtml(color)+'" '+(String(color)===String(current)?'selected':'')+'>'+escapeHtml(color)+'</option>'}).join('')+'</select></span>'}}var value=row[key];if(row._needsColor&&['item_final','item_number','item','color_code'].indexOf(key)>-1)value='';if(row._priceNeedsColor&&key==='price')value='';if(/price|rate|amount|cost|total/i.test(key)&&value!=='')value=moneyLabel(value);return '<span title="'+escapeHtml(value)+'">'+escapeHtml(value)+'</span>'}

function isLaborPricingRow(row){
  return String(row.type||'').trim().toLowerCase()==='labor'||String(row.trade||'').trim().toLowerCase()==='labor'
}
function laborPricingSourceRows(){
  return (pricingState.allRows.length?pricingState.allRows:pricingState.rows).filter(isLaborPricingRow)
}
function laborPricingFacetValues(key){
  var values={};
  laborPricingSourceRows().forEach(function(row){var value=String(row[key]||'').trim();if(value)values[value]=true});
  return Object.keys(values).sort(function(a,b){return a.localeCompare(b)})
}
function laborPricingVisibleRows(){
  var filters=laborPricingState.filters;
  var rows=laborPricingSourceRows().filter(function(row){
    if(filters.state!=='All states'&&String(row.state||'')!==filters.state)return false;
    ...
    return !search||pricingSearchText(row).indexOf(search)>-1
  });
  rows=pricingGroupedRows(rows);
  if(filters.sort==='Lowest price')rows.sort(function(a,b){return moneyNumber(a.price)-moneyNumber(b.price)});
  else if(filters.sort==='Highest price')rows.sort(function(a,b){return moneyNumber(b.price)-moneyNumber(a.price)});
  else if(filters.sort==='Supplier A-Z')rows.sort(function(a,b){return String(a.supplier||'').localeCompare(String(b.supplier||''))});
  else rows.sort(function(a,b){return String(a.elite_product_name||a.item||'').localeCompare(String(b.elite_product_name||b.item||''))});
  return rows
}
function laborPricingSelectedRow(rows){
  var selected=rows.find(function(row){return row._groupKey===laborPricingState.selectedKey});
  if(!selected)selected=rows[0]||{};
  laborPricingState.selectedKey=selected._groupKey||'';
  return selected
}
function laborPricingMetric(label,value,note){
  return '<article><span>'+escapeHtml(label)+'</span><strong>'+escapeHtml(value)+'</strong><em>'+escapeHtml(note)+'</em></article>'
}
function renderLaborPricingPage(){
  var source=laborPricingSourceRows();
  var rows=laborPricingVisibleRows();
  var selected=laborPricingSelectedRow(rows);
  var suppliers={};var states={};var missing=0;
  source.forEach(function(row){if(row.supplier)suppliers[row.supplier]=true;if(row.state)states[row.state]=true;if(row.price===''||row.price==null)missing++});
  var status=pricingState.cacheLoading?pricingState.cacheMessage:pricingState.error?pricingState.error:(pricingState.cacheReady?'Labor pricing ready':'Loading labor pricing');
  pagePanel.className='page-panel liens-page labor-pricing-page';
  pagePanel.innerHTML=renderModuleStatusLine(status)+
    '<div class="liens-metrics">'+
      laborPricingMetric('Labor Items',String(pricingGroupedRows(source).length),'Available rates')+
      laborPricingMetric('Suppliers',String(Object.keys(suppliers).length),'Represented')+
      laborPricingMetric('States',String(Object.keys(states).length),'Covered')+
      laborPricingMetric('Missing Price',String(missing),'Needs review')+
    '</div>'+
    '<section class="liens-filter-card"><div><h3>Filters + Sort + Search</h3><p>Review labor rates by supplier, state, trade, and item.</p></div>'+
      '<div class="workflow-entry-actions"><button type="button" data-labor-report>Reports</button></div>'+
      '<div class="liens-filters">'+
        '<label>State<select data-labor-filter="state">'+renderSelectOptions('All states',laborPricingFacetValues('state'),laborPricingState.filters.state)+'</select></label>'+
        '<label>Supplier<select data-labor-filter="supplier">'+renderSelectOptions('All suppliers',laborPricingFacetValues('supplier'),laborPricingState.filters.supplier)+'</select></label>'+
        '<label>Trade<select data-labor-filter="trade">'+renderSelectOptions('All trades',laborPricingFacetValues('trade'),laborPricingState.filters.trade)+'</select></label>'+
        '<label>Sort<select data-labor-filter="sort"><option'+(laborPricingState.filters.sort==='Name A-Z'?' selected':'')+'>Name A-Z</option><option'+(laborPricingState.filters.sort==='Supplier A-Z'?' selected':'')+'>Supplier A-Z</option><option'+(laborPricingState.filters.sort==='Lowest price'?' selected':'')+'>Lowest price</option><option'+(laborPricingState.filters.sort==='Highest price'?' selected':'')+'>Highest price</option></select></label>'+
        '<label>Search<input type="search" data-labor-filter="search" placeholder="Search labor pricing" value="'+escapeHtml(laborPricingState.filters.search)+'"></label>'+
      '</div></section>'+
    '<div class="liens-workspace"><section class="liens-records"><div class="liens-section-head"><div><h3>Labor Pricing</h3><p>Labor-only records from the protected pricing source.</p></div><strong>'+escapeHtml(rows.length)+' showing</strong></div>'+
      '<div class="liens-table labor-pricing-table" role="table"><div class="liens-table-head" role="row"><span>Labor Item</span><span>Supplier</span><span>State</span><span>Trade</span><span>UOM</span><span>Price</span></div>'+
      rows.map(function(row){var key=row._groupKey||pricingGroupKey(row);return '<div class="liens-row'+(key===laborPricingState.selectedKey?' active':'')+'" data-labor-key="'+escapeHtml(key)+'" role="row" tabindex="0"><span><strong>'+escapeHtml(row.elite_product_name||row.item||row.item_final||'Labor item')+'</strong><em>'+escapeHtml(row.item_number||row.item_final||'')+'</em></span><span>'+escapeHtml(row.supplier||'')+'</span><span>'+escapeHtml(row.state||'')+'</span><span>'+escapeHtml(row.trade||'')+'</span><span>'+escapeHtml(row.uom||'')+'</span><span><strong>'+escapeHtml(row.price===''||row.price==null?'Not set':moneyLabel(row.price))+'</strong></span></div>'}).join('')+
      (rows.length?'':'<div class="liens-empty">No labor pricing records match these filters.</div>')+'</div></section>'+
    '<aside class="liens-detail"><div class="liens-detail-head"><div><span>Selected labor item</span><h3>'+escapeHtml(selected.elite_product_name||selected.item||selected.item_final||'No labor item selected')+'</h3><p>'+escapeHtml([selected.supplier,selected.state].filter(Boolean).join(' / '))+'</p></div></div>'+
      '<div class="liens-detail-grid"><article><span>Supplier</span><strong>'+escapeHtml(selected.supplier||'Not set')+'</strong></article><article><span>State</span><strong>'+escapeHtml(selected.state||'Not set')+'</strong></article><article><span>Trade</span><strong>'+escapeHtml(selected.trade||'Not set')+'</strong></article><article><span>Type</span><strong>'+escapeHtml(selected.type||'Labor')+'</strong></article><article><span>UOM</span><strong>'+escapeHtml(selected.uom||'Not set')+'</strong></article><article><span>Price</span><strong>'+escapeHtml(selected.price===''||selected.price==null?'Not set':moneyLabel(selected.price))+'</strong></article><article><span>Item Number</span><strong>'+escapeHtml(selected.item_number||'Not set')+'</strong></article><article><span>Item Final</span><strong>'+escapeHtml(selected.item_final||'Not set')+'</strong></article></div>'+
      '<section class="liens-workflow"><div class="card-heading"><h3>Rate Workflow</h3><span>Pricing reference</span></div><div class="workspace-status">Confirm the labor item, supplier, state, unit of measure, and current rate before use.</div></section>'+
    '</aside></div>'
}
function openLaborPricingReportsModal(){
  openPricingReportsModal();
  var modal=document.querySelector('.citadel-modal-backdrop:last-of-type');
  if(!modal)return;
  var heading=modal.querySelector('.modal-head h3');if(heading)heading.textContent='Labor Pricing Reports';
  var description=modal.querySelector('.modal-head p');if(description)description.textContent='Build exports from labor-only pricing records.';
  var typeField=modal.querySelector('[data-pricing-report-field="type"]');
  if(typeField){
    var laborOption=Array.from(typeField.options).find(function(option){return String(option.value).toLowerCase()==='labor'});
    if(laborOption){typeField.value=laborOption.value;typeField.dispatchEvent(new Event('change',{bubbles:true}))}
  }
}
function bindLaborPricingPage(){
  if(pagePanel.getAttribute('data-labor-pricing-bound')==='true')return;
  pagePanel.setAttribute('data-labor-pricing-bound','true');
  pagePanel.addEventListener('click',function(event){
    if(activePage!=='labor-pricing')return;
    if(event.target.closest('[data-labor-report]')){openLaborPricingReportsModal();return}
    var row=event.target.closest('[data-labor-key]');
    if(row){laborPricingState.selectedKey=row.getAttribute('data-labor-key')||'';renderLaborPricingPage();bindLaborPricingPage()}
  });
  pagePanel.addEventListener('change',function(event){
    if(activePage!=='labor-pricing')return;
    var field=event.target.closest('select[data-labor-filter]');if(!field)return;
    laborPricingState.filters[field.getAttribute('data-labor-filter')]=field.value;laborPricingState.selectedKey='';renderLaborPricingPage();bindLaborPricingPage()
  });
  pagePanel.addEventListener('input',function(event){
    if(activePage!=='labor-pricing')return;
    var field=event.target.closest('input[data-labor-filter="search"]');if(!field)return;
...
function renderTemplateLineItems(title,items){
  ...
  return '<section class="liens-workflow template-line-section"><div class="card-heading"><h3>'+escapeHtml(title)+'</h3><span>'+escapeHtml(items.length)+' items</span></div>'+
    (items.length?'<div class="template-line-list">'+items.map(function(item){return '<article><div><strong>'+escapeHtml(item.name)+'</strong>'+(item.rules?'<p>'+escapeHtml(item.rules)+'</p>':'')+'</div><span>'+escapeHtml(item.uom||'â€”')+'</span></article>'}).join('')+'</div>':'<p class="liens-empty">No '+escapeHtml(title.toLowerCase())+' included.</p>')+
  '</section>'
}
...
function workflowRowsForModule(moduleId){
  ...
  var alertRows=(source.alerts||[]).filter(workflowItemIsOpen).map(function(item){
    return {source:source.label,text:item.alert_text||item.alert_type||'Alert',meta:[item.owner||'Team',item.due_date||item.created_date||'No due date'].join(' - ')};
  });
...
function lienImportStatusMarkup(status){
  ...
  var latestLabel=latest.status?(latest.status+(latest.completed_at?" Â· "+latest.completed_at:"")):"No completed Liens import yet";
  var warningMarkup=(status.warnings||[]).map(function(item){return '<div class="report-callout">'+escapeHtml(item)+'</div>'}).join("");
  var errorMarkup=(status.errors||[]).map(function(item){return '<div class="report-callout"><strong>Check:</strong> '+escapeHtml(item)+'</div>'}).join("");
...
function bindLiensPage(){
  ...
  pagePanel.addEventListener("click",function(event){
    ...
    if(reportButton){openLiensReportsModal();return}
...
function renderAssignedAttorneyOptions(selectedId,selectedName){
  ...
  var options=attorneys.map(function(contact){
    ...
    if(organizationCounts[organization]>1&&contact.contactName&&contact.contactName!==organization)label+=' â€” '+contact.contactName;
...
function renderRegionHealthPage(){
  ...
  pagePanel.innerHTML=renderModuleStatusLine(connectedSources+' / 4 mapped sources live - Reviews excluded until a region field is connected')+
    ...
    '<div class="region-health-metrics"><article><span>Open Alerts</span><strong>'+escapeHtml(openAlerts)+'</strong><em>live alerts + follow-ups</em></article><article><span>Regions</span><strong>'+escapeHtml(scoredRows.length)+'</strong><em>with live signals</em></article><article><span>Healthy</span><strong>'+escapeHtml(healthy)+'</strong><em>operating well</em></article><article><span>Watchlist</span><strong>'+escapeHtml(watch)+'</strong><em>needs follow-up</em></article><article><span>Critical</span><strong>'+escapeHtml(critical)+'</strong><em>leadership focus</em></article></div>'+
