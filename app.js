...
function renderCollectionRecordDetail(selected){
  var tracking='<form class="collection-tracking-form" data-collection-tracking-form><input type="hidden" name="collection_id" value="'+escapeHtml(selected.id)+'"><input type="hidden" name="lien_id" value="'+escapeHtml(selected.lienId)+'"><div class="collection-tracking-form-grid"><label>Collection Agency<input name="collection_agency" list="collection-agency-options" value="'+escapeHtml(selected.agency)+'"><datalist id="collection-agency-options">'+getCollectionAgencies().map(function(value){return '<option value="'+escapeHtml(value)+'">'}).join('')+'</datalist></label><label>Date Sent to Agency<input name="date_sent_to_agency" type="date" value="'+escapeHtml(selected.sentDate)+'"></label><label>Amount Outstanding<input name="amount_outstanding" type="number" min="0" step="0.01" value="'+escapeHtml(selected.amountOutstanding)+'"></label><label>Amount Collected<input name="amount_collected" type="number" min="0" step="0.01" value="'+escapeHtml(selected.amountCollected||'')+'"></label><label>Amount We Will Receive<input name="amount_we_receive" type="number" min="0" step="0.01" value="'+escapeHtml(selected.amountWeReceive||'')+'"></label><label>Date Received<input name="date_received" type="date" value="'+escapeHtml(selected.dateReceived)+'"></label></div><section class="collection-assignment-panel"><div><h4>Assigned Contacts & Attorneys</h4><p>Select one or more saved attorneys, agencies, company, office, or customer contacts.</p></div>'+renderCollectionContactChoices(selected)+'</section><button type="submit" class="collection-save-tracking">Save Collection Tracking</button></form>';
