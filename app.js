...
function loadPricingStaticData(){var data=window.CITADEL_PRICING_DATA;if(!data||!data.rows||!data.rows.length)return false;pricingState.headers=data.headers||[];pricingState.keys=data.keys||Object.keys
Exit code: 0
Wall time: 2.2 seconds
Output:
(data.rows[0]||{});pricingState.allRows=data.rows||[];pricingState.sheet='Pricing snapshot';pricingState.cacheReady=true;pricingState.loading=false;pricingState.cacheLoading=false;pricingState.error='';pricingState.cacheMessage='Pricing snapshot ready';refreshPricingFacets();applyPricingLocalView();return true}
...
var STANDARD_FILTERS={
  ...
  "region-health":[["Region","All regions",["Wisconsin","St. Louis","I
Exit code: 0
Wall time: 2.3 seconds
Output:
ndianapolis","Chicago","Santa Fe"]],["Health","All health",["Healthy","Watchlist","Critical","No Coverage"]],["Coverage","All coverage",["Covered","Gap","Low Capacity","Escalated"]],["Sort","Needs attention",["Needs attention","Coverage low","Region name"]]]
};
...
function renderLienPaymentRows(rows,limit){
  ...
  return '<div class="lien-payment-list">'+items.map(function(item){
    ...
    return '<article class="'+classes.join(' ')+'"><div><strong>'+escapeHtml(formatLienPaymentDate(item.date))+'</strong><span>'+escapeHtml([item.type,item.method].filter(Boolean).join(' / ')||'Payment')+'</span></div><strong class="lien-payment-amount">'+escapeHtml(moneyLabel(item.amount))+'</strong><e
Exit code: 0
Wall time: 2.3 seconds
Output:
m>'+escapeHtml(lienPaymentStatusLabel(item))+'</em></article>'
  }).join('')+'</div>'
}
...
function contractorRiskScore(row){var risk=String(row.risk||"");var score=Number((risk.match(/\d+/)||[0])[0]);if(score)return score;if(/expired/i.test(row.document
Exit code: 0
Wall time: 2.5 seconds
Output:
s))return 70;if(/expiring|review/i.test(row.documents))return 50;return 20}
...
Wall time: 2.5 seconds
Output:
idden" name="collection_id" value="'+escapeHtml(selected.id)+'">'+
...
function supplierAgreementMissing(row){return !/^(signed|not 
Exit code: 0
Wall time: 2.4 seconds
Output:
required)$/i.test(String(row.agreementStatus||''))}
