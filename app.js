...
function normalizeCollectionRecord(record){
  return {
    ...
    balanceSentToAgency:record.balance_sent_to_agency||record.amount_outstanding||0,
...
function renderCollectionAccountsWorkspace(){
  ...
  var detail=selected?'<aside class="liens-detail collections-detail"><div class="liens-detail-head"><div><span>Selected collection job</span><h3>'+escapeHtml(selected.jobNumber||selected.id)+' - '+escapeHtml(selected.customer)+'</h3><p>'+escapeHtml(selected.currentStage||'Collections')+' / '+escapeHtml(selected.status)+'</p></div><button type="button" data-edit-collection>Edit</button></div><div class="liens-detail-grid"><article><span>Region</span><strong>'+escapeHtml(selected.region||'-')+'</strong></article><article><span>Sales Rep</span><strong>'+escapeHtml(selected.salesRep||'-')+'</strong></article><article><span>Current Stage</span><strong>'+escapeHtml(selected.currentStage||'-')+'</strong></article><article><span>Aging</span><strong>'+escapeHtml(selected.agingDays+' days')+'</strong></article><article><span>Total Revenue</span><strong>'+escapeHtml(moneyLabel(selected.totalRevenue))+'</strong></article><article><span>Payments</span><strong>'+escapeHtml(moneyLabel(selected.paymentsReceived))+'</strong></article><article><span>Balance Sent to Agency</span><strong>'+escapeHtml(moneyLabel(selected.balanceSentToAgency))+'</strong></article><article><span>Collection Agency</span><strong>'+escapeHtml(selected.agency||'-')+'</strong></article><article><span>Date Sent to Agency</span><strong>'+escapeHtml(selected.sentDate||'-')+'</strong></article><article><span>Amount Outstanding</span><strong>'+escapeHtml(moneyLabel(selected.amountOutstanding))+'</strong></article><article><span>Amount Collected</span><strong>'+escapeHtml(moneyLabel(selected.amountCollected))+'</strong></article><article><span>Amount We Receive</span><strong>'+escapeHtml(moneyLabel(selected.amountWeReceive))+'</strong></article><article><span>Date Received</span><strong>'+escapeHtml(selected.dateReceived||'-')+'</strong></article><article><span>Job Link</span><strong>'+collectionJobLinkHtml(selected)+'</strong></article><article><span>Status</span><strong>'+escapeHtml(selected.status)+'</strong></article></div><section class="collection-contact-panel"><div class="card-heading"><h3>Related Contacts & Attorneys</h3><span>Reusable protected directory</span></div>'+renderCollectionRelatedContacts(selected)+'</section><section class="liens-workflow"><div class="card-heading"><h3>Citadel Workflow</h3><span>Protected from source imports</span></div><div class="workspace-status">'+escapeHtml(collectionsWorkspaceStatus)+'</div><div class="workflow-entry-actions"><button type="button" data-workspace-entry="note">Add Note</button><button type="button" data-workspace-entry="alert">Set Alert</button></div><div class="liens-workspace-feed"><h4>Recent Notes</h4>'+renderLienWorkspaceList(getCollectionWorkspaceItems('notes',selected.id),'No notes yet.',[{key:'note_text',strong:true},{key:'note_by',fallback:'Team'},{key:'note_date'}])+'<h4>Open Alerts</h4>'+renderLienWorkspaceList(getCollectionWorkspaceItems('alerts',selected.id),'No alerts yet.',[{key:'alert_text',strong:true},{key:'owner',fallback:'Carlynn'},{key:'due_date'}])+'</div></section></aside>':'<aside class="liens-detail collections-detail"><div class="collections-empty-detail"><h3>No collection jobs yet</h3><p>Add the first job sent to collections, then link saved attorneys, agencies, company, office, and customer contacts.</p><button type="button" data-add-collection>Add Collection Job</button></div></aside>';
...
function openCollectionRecordModal(record){
  ...
  modal.innerHTML='<section class="workspace-entry-modal collection-record-modal" role="dialog" aria-modal="true" aria-label="Collection Job"><div class="modal-head"><div><h3>'+(record?'Edit Collection Job':'Add Collection Job')+'</h3><p>Protect the source job, agency placement, recovery, and linked contacts in Google Sheets.</p></div><button type="button" data-close-modal aria-label="Close">x</button></div><form data-collection-record-form><div class="collection-record-form-grid"><label>Region<input name="region" value="'+escapeHtml(data.region||'')+'"></label><label>Sales Rep<input name="sales_rep" value="'+escapeHtml(data.salesRep||'')+'"></label><label>Job Number *<input name="job_number" required value="'+escapeHtml(data.jobNumber||'')+'"></label><label>Job Link<input name="job_link" type="url" placeholder="https://" value="'+escapeHtml(data.jobLink||'')+'"></label><label>Customer *<input name="customer" required value="'+escapeHtml(data.customer||'')+'"></label><label>Current Stage<input name="current_stage" value="'+escapeHtml(data.currentStage||'')+'"></label><label>Aging Days<input name="aging_days" type="number" min="0" step="1" placeholder="Calculated from date sent" value="'+escapeHtml(data.agingDays||'')+'"></label><label>Total Revenue<input name="total_revenue" type="number" min="0" step="0.01" value="'+escapeHtml(data.totalRevenue||'')+'"></label><label>Payments Received<input name="payments_received" type="number" min="0" step="0.01" value="'+escapeHtml(data.paymentsReceived||'')+'"></label><label>Balance Sent to Agency<input name="balance_sent_to_agency" type="number" min="0" step="0.01" value="'+escapeHtml(data.balanceSentToAgency||'')+'"></label><label>Collection Agency *<input name="collection_agency" required list="collection-agency-options" value="'+escapeHtml(data.agency||'')+'"><datalist id="collection-agency-options">'+getCollectionAgencies().map(function(value){return '<option value="'+escapeHtml(value)+'">'}).join('')+'</datalist></label><label>Date Sent to Agency *<input name="date_sent_to_agency" type="date" required value="'+escapeHtml(data.sentDate||'')+'"></label><label>Amount Outstanding<input name="amount_outstanding" type="number" min="0" step="0.01" placeholder="Calculated when blank" value="'+escapeHtml(data.amountOutstanding||'')+'"></label><label>Amount Collected<input name="amount_collected" type="number" min="0" step="0.01" value="'+escapeHtml(data.amountCollected||'')+'"></label><label>Amount We Will Receive<input name="amount_we_receive" type="number" min="0" step="0.01" value="'+escapeHtml(data.amountWeReceive||'')+'"></label><label>Date Received<input name="date_received" type="date" value="'+escapeHtml(data.dateReceived||'')+'"></label></div><section class="collection-related-section"><div><h4>Related Contacts & Attorneys</h4><p>Select one or more saved attorneys, agencies, companies, offices, and customer contacts.</p></div>'+renderCollectionContactChoices(record)+'</section><div class="workspace-entry-actions"><button type="button" data-close-modal>Cancel</button><button type="submit" class="primary">Save Collection Job</button></div></form></section>';
  modal.addEventListener('click',function(event){if(event.target===modal||event.target.closest('[data-close-modal]'))closeLiensReportsModal()});
...
function setQuickCollectionsReport(modal,type){
  ...
  if(!contacts&&type==='open')modal.querySelector('[data-collection-report-filter="status"]').value='Open';
...
function collectionReportTable(rows,contacts){
  return contacts
    ...
    :[['Region','Sales Rep','Job Number','Job Link','Customer','Current Stage','Aging Days','Total Revenue','Payments Received','Balance Sent to Agency','Collection Agency','Date Sent to Agency','Amount Outstanding','Amount Collected','Amount We Will Receive','Date Received','Status','Updated By','Last Updated']].concat(rows.map(function(row){return [row.region,row.salesRep,row.jobNumber,row.jobLink,row.customer,row.currentStage,row.agingDays,moneyNumber(row.totalRevenue),moneyNumber(row.paymentsReceived),moneyNumber(row.balanceSentToAgency),row.agency,row.sentDate,moneyNumber(row.amountOutstanding),moneyNumber(row.amountCollected),moneyNumber(row.amountWeReceive),row.dateReceived,row.status,row.updatedBy,row.lastUpdated]}))
}
