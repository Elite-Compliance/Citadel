...
var collectionsData={records:[],notes:[],alerts:[],contacts:[],contactLinks:[],metrics:[],selectedIndex:0,selectedContactIndex:0};
...
var collectionFilters={agency:"All agencies",status:"All statuses",region:"All regions",sort:"Highest outstanding",search:""};
...
function getCommandCenterModules(){
  ...
  var sourceModules=[
    ...
    liveCommandModule('collections','Collections',collectionState,collectionRecords.length+' collection accounts',moneyLabel(collectionRecords.reduce(function(sum,row){return sum+moneyNumber(row.amountOutstanding)},0))+' outstanding',workflowRowsForModule('collections').length,collectionsLoadError),
...
function getWorkspaceEntryConfig(section){
  if(section==="collections"){
    ...
    return {id:collection.id,idField:"collection_id",label:[collection.accountId,collection.customer].filter(Boolean).join(" - "),notePlaceholder:"Add a protected collection note",noteAction:"saveCollectionNote",alertAction:"saveCollectionAlert",save:saveCollectionWorkspace}
  }
...
function applyCollectionsPayload(data){
  ...
  collectionsData.contacts=(data.contacts||[]).map(normalizeBusinessContact);
...
function getCollectionContactTypes(){return Array.from(new Set((collectionsData.contacts||[]).map(function(row){return row.contactType}).filter(Boolean))).sort()}
...
function getCollectionMetricStats(){
  ...
  return [
    ...
    {key:'accounts',label:'Accounts',value:records.length,note:'Sent to collections'},
    {key:'outstanding',label:'Outstanding',value:moneyLabel(outstanding),note:'Amount placed'},
    ...
    {key:'contacts',label:'Business Contacts',value:(collectionsData.contacts||[]).length,note:'Reusable directory'}
  ]
}
function getVisibleCollectionRecords(){
  ...
  if(collectionFilters.status!=='All statuses')rows=rows.filter(function(row){return row.status===collectionFilters.status});
  ...
  if(query)rows=rows.filter(function(row){return [row.accountId,row.customer,row.region,row.agency,row.status,row.sentDate,row.dateReceived].join(' ').toLowerCase().indexOf(query)>-1});
  ...
  if(collectionFilters.sort==='Recently received')rows.sort(function(a,b){return String(b.dateReceived).localeCompare(String(a.dateReceived))});
  return rows
}
function getVisibleBusinessContacts(){
  ...
  if(query)rows=rows.filter(function(row){return [row.contactType,row.organizationName,row.contactName,row.jobTitle,row.primaryEmail,row.secondaryEmail,row.officePhone,row.mobilePhone,row.fax,row.website,row.businessAddress1,row.businessCity,row.businessState,row.businessZip].join(' ').toLowerCase().indexOf(query)>-1});
...
function formatBusinessAddress(contact,mailing){
  ...
  var lines=[contact[prefix+'Address1'],contact[prefix+'Address2'],[contact[prefix+'City'],contact[prefix+'State'],contact[prefix+'Zip']].filter(Boolean).join(', ').replace(', ,',','),contact[prefix+'Country']].filter(Boolean);
  return lines.join(' / ')
}
function renderCollectionRelatedContacts(record){
  var links=getCollectionLinks(record.id);
  if(!links.length)return '<p class="liens-empty">No business contacts linked yet.</p>';
  var grouped={};
...
function renderCollectionFilters(){
  if(collectionView==='contacts')return '<div class="liens-filters collections-contact-filters"><label>Contact Type<select data-contact-filter="type">'+renderSelectOptions('All contact types',getCollectionContactTypes(),contactFilters.type)+'</select></label><label>State<select data-contact-filter="state">'+renderSelectOptions('All states',getCollectionContactStates(),contactFilters.state)+'</select></label><label>Sort<select data-contact-filter="sort"><option'+(contactFilters.sort==='Organization A-Z'?' selected':'')+'>Organization A-Z</option><option'+(contactFilters.sort==='Contact A-Z'?' selected':'')+'>Contact A-Z</option><option'+(contactFilters.sort==='Recently updated'?' selected':'')+'>Recently updated</option></select></label><label>Search<input data-contact-filter="search" type="search" placeholder="Search contacts" value="'+escapeHtml(contactFilters.search)+'"></label></div>';
  return '<div class="liens-filters"><label>Agency<select data-collection-filter="agency">'+renderSelectOptions('All agencies',getCollectionAgencies(),collectionFilters.agency)+'</select></label><label>Status<select data-collection-filter="status">'+renderSelectOptions('All statuses',['Open','Partially Collected','Received'],collectionFilters.status)+'</select></label><label>Region<select data-collection-filter="region">'+renderSelectOptions('All regions',getCollectionRegions(),collectionFilters.region)+'</select></label><label>Sort<select data-collection-filter="sort"><option'+(collectionFilters.sort==='Highest outstanding'?' selected':'')+'>Highest outstanding</option><option'+(collectionFilters.sort==='Highest collected'?' selected':'')+'>Highest collected</option><option'+(collectionFilters.sort==='Highest expected'?' selected':'')+'>Highest expected</option><option'+(collectionFilters.sort==='Newest sent'?' selected':'')+'>Newest sent</option><option'+(collectionFilters.sort==='Recently received'?' selected':'')+'>Recently received</option></select></label><label>Search<input data-collection-filter="search" type="search" placeholder="Search this view" value="'+escapeHtml(collectionFilters.search)+'"></label></div>'
}
function renderCollectionAccountsWorkspace(){
  var rows=getVisibleCollectionRecords();
  var selected=collectionsData.records[collectionsData.selectedIndex]||rows[0]||collectionsData.records[0];
  var table='<div class="collections-table" role="table"><div class="collections-table-head" role="row"><span>Account / Customer</span><span>Agency</span><span>Date Sent</span><span>Outstanding</span><span>Collected</span><span>We Receive</span></div>'+(rows.length?rows.map(function(record){var index=collectionsData.records.indexOf(record);return '<div class="collections-row'+(index===collectionsData.selectedIndex?' active':'')+'" data-collection-index="'+index+'" role="row" tabindex="0"><span><strong>'+escapeHtml(record.accountId||record.id)+'</strong><em>'+escapeHtml(record.customer)+'</em></span><span>'+escapeHtml(record.agency)+'</span><span>'+escapeHtml(record.sentDate)+'</span><span>'+escapeHtml(moneyLabel(record.amountOutstanding))+'</span><span>'+escapeHtml(moneyLabel(record.amountCollected))+'</span><span>'+escapeHtml(moneyLabel(record.amountWeReceive))+'</span></div>'}).join(''):'<p class="collections-empty-table">No collection accounts match this view.</p>')+'</div>';
  var detail=selected?'<aside class="liens-detail collections-detail"><div class="liens-detail-head"><div><span>Selected collection</span><h3>'+escapeHtml(selected.accountId||selected.id)+' - '+escapeHtml(selected.customer)+'</h3><p>Collections / '+escapeHtml(selected.status)+'</p></div><button type="button" data-edit-collection> Edit </button></div><div class="liens-detail-grid"><article><span>Region</span><strong>'+escapeHtml(selected.region||'—')+'</strong></article><article><span>Collection Agency</span><strong>'+escapeHtml(selected.agency||'—')+'</strong></article><article><span>Date Sent</span><strong>'+escapeHtml(selected.sentDate||'—')+'</strong></article><article><span>Outstanding</span><strong>'+escapeHtml(moneyLabel(selected.amountOutstanding))+'</strong></article><article><span>Amount Collected</span><strong>'+escapeHtml(moneyLabel(selected.amountCollected))+'</strong></article><article><span>Amount We Receive</span><strong>'+escapeHtml(moneyLabel(selected.amountWeReceive))+'</strong></article><article><span>Date Received</span><strong>'+escapeHtml(selected.dateReceived||'—')+'</strong></article><article><span>Status</span><strong>'+escapeHtml(selected.status)+'</strong></article></div><section class="collection-contact-panel"><div class="card-heading"><h3>Related Business Contacts</h3><span>Reusable directory</span></div>'+renderCollectionRelatedContacts(selected)+'</section><section class="liens-workflow"><div class="card-heading"><h3>Citadel Workflow</h3><span>Protected from source imports</span></div><div class="workspace-status">'+escapeHtml(collectionsWorkspaceStatus)+'</div><div class="workflow-entry-actions"><button type="button" data-workspace-entry="note">Add Note</button><button type="button" data-workspace-entry="alert">Set Alert</button></div><div class="liens-workspace-feed"><h4>Recent Notes</h4>'+renderLienWorkspaceList(getCollectionWorkspaceItems('notes',selected.id),'No notes yet.',[{key:'note_text',strong:true},{key:'note_by',fallback:'Team'},{key:'note_date'}])+'<h4>Open Alerts</h4>'+renderLienWorkspaceList(getCollectionWorkspaceItems('alerts',selected.id),'No alerts yet.',[{key:'alert_text',strong:true},{key:'owner',fallback:'Carlynn'},{key:'due_date'}])+'</div></section></aside>':'<aside class="liens-detail collections-detail"><div class="collections-empty-detail"><h3>No collection accounts yet</h3><p>Add the first job sent to collections, then link saved attorneys, agencies, companies, offices, and customer contacts.</p><button type="button" data-add-collection>Add Collection</button></div></aside>';
  return '<div class="liens-workspace"><section class="liens-records"><div class="liens-section-head"><div><h3>Collection Accounts</h3><p>Track jobs sent to a collection agency and the recovery received.</p></div><strong>'+escapeHtml(rows.length)+' showing</strong></div>'+table+'</section>'+detail+'</div>'
}
function renderBusinessContactsWorkspace(){
  ...
  var table='<div class="business-contacts-table" role="table"><div class="business-contacts-head" role="row"><span>Type</span><span>Organization</span><span>Contact</span><span>Office Phone</span><span>Email</span><span>Location</span></div>'+(rows.length?rows.map(function(contact){var index=collectionsData.contacts.indexOf(contact);return '<div class="business-contact-row'+(index===collectionsData.selectedContactIndex?' active':'')+'" data-contact-index="'+index+'" role="row" tabindex="0"><span>'+escapeHtml(contact.contactType)+'</span><span>'+escapeHtml(contact.organizationName||'—')+'</span><span><strong>'+escapeHtml(contact.contactName||'—')+'</strong><em>'+escapeHtml(contact.jobTitle)+'</em></span><span>'+escapeHtml(contact.officePhone||contact.mobilePhone||'—')+'</span><span>'+escapeHtml(contact.primaryEmail||'—')+'</span><span>'+escapeHtml([contact.businessCity,contact.businessState].filter(Boolean).join(', ')||'—')+'</span></div>'}).join(''):'<p class="collections-empty-table">No business contacts match this view.</p>')+'</div>';
  var detail=selected?'<aside class="liens-detail collections-detail"><div class="liens-detail-head"><div><span>'+escapeHtml(selected.contactType)+'</span><h3>'+escapeHtml(selected.organizationName||selected.contactName)+'</h3><p>'+escapeHtml([selected.contactName,selected.jobTitle,selected.department].filter(Boolean).join(' - '))+'</p></div><button type="button" data-edit-contact>Edit</button></div><div class="liens-detail-grid"><article><span>Office Phone</span><strong>'+escapeHtml(selected.officePhone||'—')+(selected.phoneExtension?' ext. '+escapeHtml(selected.phoneExtension):'')+'</strong></article><article><span>Mobile Phone</span><strong>'+escapeHtml(selected.mobilePhone||'—')+'</strong></article><article><span>Primary Email</span><strong>'+escapeHtml(selected.primaryEmail||'—')+'</strong></article><article><span>Secondary Email</span><strong>'+escapeHtml(selected.secondaryEmail||'—')+'</strong></article><article><span>Fax</span><strong>'+escapeHtml(selected.fax||'—')+'</strong></article><article><span>Website</span><strong>'+escapeHtml(selected.website||'—')+'</strong></article><article><span>Preferred Contact</span><strong>'+escapeHtml(selected.preferredContactMethod||'—')+'</strong></article></div><section class="collection-contact-panel"><div class="card-heading"><h3>Address Details</h3><span>Business and mailing</span></div><div class="collection-address-grid"><article><span>Business Address</span><strong>'+escapeHtml(formatBusinessAddress(selected,false)||'—')+'</strong></article><article><span>Mailing Address</span><strong>'+escapeHtml(formatBusinessAddress(selected,true)||'—')+'</strong></article></div>'+(selected.notes?'<div class="collection-contact-notes"><span>Notes</span><p>'+escapeHtml(selected.notes)+'</p></div>':'')+'</section></aside>':'<aside class="liens-detail collections-detail"><div class="collections-empty-detail"><h3>No business contacts yet</h3><p>Add attorneys, collection agencies, company contacts, office contacts, and customer contacts once, then reuse them on any collection job.</p><button type="button" data-add-contact>Add New Contact</button></div></aside>';
  return '<div class="liens-workspace"><section class="liens-records"><div class="liens-section-head"><div><h3>Business Contacts</h3><p>A reusable directory for every party involved in collection work.</p></div><strong>'+escapeHtml(rows.length)+' showing</strong></div>'+table+'</section>'+detail+'</div>'
}
function renderCollectionsPage(){
  if(collectionsLoading&&!collectionsData.records.length&&!collectionsData.contacts.length){pagePanel.className='page-panel liens-page collections-page';pagePanel.innerHTML='<section class="liens-loading-card"><div class="loading-pulse"></div><h3>Loading Collections</h3><p>Connecting to the protected collection records and business-contact directory.</p></section>';return}
  ...
  pagePanel.innerHTML=renderModuleStatusLine(statusText)+'<div class="liens-metrics collections-metrics">'+metrics.map(function(metric){return '<button type="button" class="lien-metric-button '+(activeCollectionMetric===metric.key?'active':'')+'" data-collection-metric="'+escapeHtml(metric.key)+'"><span>'+escapeHtml(metric.label)+'</span><strong>'+escapeHtml(metric.value)+'</strong><em>'+escapeHtml(metric.note)+'</em></button>'}).join('')+'</div><div class="collections-view-tabs"><button type="button" class="'+(collectionView==='accounts'?'active':'')+'" data-collection-view="accounts">Collection Accounts</button><button type="button" class="'+(collectionView==='contacts'?'active':'')+'" data-collection-view="contacts">Business Contacts</button></div><section class="liens-filter-card collections-filter-card"><div><h3>Filters + Sort + Search</h3><p>'+(collectionView==='contacts'?'Find and reuse business contacts across collection jobs.':'Track every account from agency placement through receipt.')+'</p></div><div class="collections-filter-actions"><button type="button" data-collection-reports>Reports</button><button type="button" data-add-contact>Add New Contact</button><button type="button" data-add-collection>Add Collection</button></div>'+renderCollectionFilters()+'</section>'+(collectionView==='contacts'?renderBusinessContactsWorkspace():renderCollectionAccountsWorkspace())
}
function businessContactPayload(contact){return contact?{contact_id:contact.id,contact_type:contact.contactType,organization_name:contact.organizationName,contact_name:contact.contactName,job_title:contact.jobTitle,department:contact.department,primary_email:contact.primaryEmail,secondary_email:contact.secondaryEmail,office_phone:contact.officePhone,phone_extension:contact.phoneExtension,mobile_phone:contact.mobilePhone,fax:contact.fax,website:contact.website,preferred_contact_method:contact.preferredContactMethod,business_address_1:contact.businessAddress1,business_address_2:contact.businessAddress2,business_city:contact.businessCity,business_state:contact.businessState,business_zip:contact.businessZip,business_country:contact.businessCountry,mailing_same_as_business:contact.mailingSameAsBusiness,mailing_address_1:contact.mailingAddress1,mailing_address_2:contact.mailingAddress2,mailing_city:contact.mailingCity,mailing_state:contact.mailingState,mailing_zip:contact.mailingZip,mailing_country:contact.mailingCountry,notes:contact.notes}:{} }
function openBusinessContactModal(contact){
  ...
  modal.addEventListener('change',function(event){if(event.target.name==='mailing_same_as_business')syncMailing()});
  modal.querySelector('[data-business-contact-form]').addEventListener('submit',function(event){event.preventDefault();var form=event.currentTarget;if(form.elements.contact_type.value==='Select type'){form.elements.contact_type.focus();return}if(!form.elements.organization_name.value.trim()&&!form.elements.contact_name.value.trim()){form.elements.organization_name.setCustomValidity('Enter an organization or contact name.');form.elements.organization_name.reportValidity();form.elements.organization_name.addEventListener('input',function(){this.setCustomValidity('')},{once:true});return}var payload={};new FormData(form).forEach(function(value,key){payload[key]=value});if(contact)payload.contact_id=contact.id;payload.mailing_same_as_business=form.elements.mailing_same_as_business.checked;saveCollectionApiAction('saveBusinessContact',payload,'Contact saved');closeLiensReportsModal()});
  document.body.appendChild(modal);syncMailing();modal.querySelector('select').focus()
}
function renderCollectionContactChoices(record){
  if(!collectionsData.contacts.length)return '<p class="collections-contact-empty">No business contacts have been saved yet. Save the job now, then add contacts and edit this job to link them.</p>';
  ...
  return '<label class="collection-contact-search">Find a saved contact<input type="search" data-related-contact-search placeholder="Attorney, agency, company, office, customer..."></label><div class="collection-contact-chooser">'+Object.keys(grouped).sort().map(function(role){return '<fieldset data-contact-choice-group><legend>'+escapeHtml(role)+'</legend>'+grouped[role].map(function(contact){var key=String(contact.id)+'|'+role;var search=[role,contact.organizationName,contact.contactName,contact.officePhone,contact.primaryEmail].join(' ').toLowerCase();return '<label data-contact-choice-row data-contact-search-text="'+escapeHtml(search)+'"><input type="checkbox" data-related-contact="'+escapeHtml(contact.id)+'" data-contact-relationship="'+escapeHtml(role)+'" '+(selected[key]?'checked':'')+'><span><strong>'+escapeHtml(contact.organizationName||contact.contactName)+'</strong><em>'+escapeHtml([contact.contactName,contact.officePhone,contact.primaryEmail].filter(Boolean).join(' | '))+'</em></span></label>'}).join('')+'</fieldset>'}).join('')+'</div>'
}
function openCollectionRecordModal(record){
  closeLiensReportsModal();
  var data=record||{};var modal=document.createElement('div');modal.className='citadel-modal-backdrop';
  modal.innerHTML='<section class="workspace-entry-modal collection-record-modal" role="dialog" aria-modal="true" aria-label="Collection Account"><div class="modal-head"><div><h3>'+(record?'Edit Collection Account':'Add Collection Account')+'</h3><p>Track the job and select every related business contact.</p></div><button type="button" data-close-modal aria-label="Close">x</button></div><form data-collection-record-form><div class="collection-record-form-grid"><label>Account / Job Number *<input name="account_id" required value="'+escapeHtml(data.accountId||'')+'"></label><label>Customer *<input name="customer" required value="'+escapeHtml(data.customer||'')+'"></label><label>Region<input name="region" value="'+escapeHtml(data.region||'')+'"></label><label>Collection Agency *<input name="collection_agency" list="collection-agency-options" value="'+escapeHtml(data.agency||'')+'"><datalist id="collection-agency-options">'+getCollectionAgencies().map(function(value){return '<option value="'+escapeHtml(value)+'">'}).join('')+'</datalist></label><label>Date Sent *<input name="sent_date" type="date" required value="'+escapeHtml(data.sentDate||'')+'"></label><label>Amount Outstanding<input name="amount_outstanding" type="number" min="0" step="0.01" value="'+escapeHtml(data.amountOutstanding||'')+'"></label><label>Amount Collected<input name="amount_collected" type="number" min="0" step="0.01" value="'+escapeHtml(data.amountCollected||'')+'"></label><label>Amount We Will Receive<input name="amount_we_receive" type="number" min="0" step="0.01" value="'+escapeHtml(data.amountWeReceive||'')+'"></label><label>Date Received<input name="date_received" type="date" value="'+escapeHtml(data.dateReceived||'')+'"></label></div><section class="collection-related-section"><div><h4>Related Business Contacts</h4><p>Select one or more saved companies, attorneys, offices, customers, and collection-agency contacts.</p></div>'+renderCollectionContactChoices(record)+'</section><div class="workspace-entry-actions"><button type="button" data-close-modal>Cancel</button><button type="submit" class="primary">Save Collection</button></div></form></section>';
  modal.addEventListener('click',function(event){if(event.target===modal||event.target.closest('[data-close-modal]'))closeLiensReportsModal()});
  modal.addEventListener('input',function(event){if(!event.target.matches('[data-related-contact-search]'))return;var query=event.target.value.trim().toLowerCase();modal.querySelectorAll('[data-contact-choice-row]').forEach(function(row){row.hidden=!!query&&String(row.getAttribute('data-contact-search-text')||'').indexOf(query)<0});modal.querySelectorAll('[data-contact-choice-group]').forEach(function(group){group.hidden=!Array.from(group.querySelectorAll('[data-contact-choice-row]')).some(function(row){return !row.hidden})})});
  modal.querySelector('[data-collection-record-form]').addEventListener('submit',function(event){event.preventDefault();var form=event.currentTarget;var payload={};new FormData(form).forEach(function(value,key){payload[key]=value});if(record)payload.collection_id=record.id;payload.contact_links=JSON.stringify(Array.from(form.querySelectorAll('[data-related-contact]:checked')).map(function(input){return {contact_id:input.getAttribute('data-related-contact'),relationship:input.getAttribute('data-contact-relationship'),is_primary:false}}));if(!payload.collection_agency){var agencyContact=Array.from(form.querySelectorAll('[data-contact-relationship="Collection Agency"]:checked'))[0];var contact=agencyContact&&getBusinessContact(agencyContact.getAttribute('data-related-contact'));payload.collection_agency=contact?contact.organizationName:''}saveCollectionApiAction('saveCollectionRecord',payload,'Collection saved');closeLiensReportsModal()});
...
function bindCollectionsPage(){
  ...
  pagePanel.addEventListener('click',function(event){
    ...
    var metric=event.target.closest('[data-collection-metric]');if(metric){activeCollectionMetric=metric.getAttribute('data-collection-metric');collectionView=activeCollectionMetric==='contacts'?'contacts':'accounts';collectionsData.selectedIndex=0;collectionsData.selectedContactIndex=0;renderCollectionsPage();return}
    var view=event.target.closest('[data-collection-view]');if(view){collectionView=view.getAttribute('data-collection-view');activeCollectionMetric=collectionView==='contacts'?'contacts':'accounts';renderCollectionsPage();return}
    ...
    if(event.target.closest('[data-add-collection]')){openCollectionRecordModal();return}
    if(event.target.closest('[data-edit-collection]')){openCollectionRecordModal(collectionsData.records[collectionsData.selectedIndex]);return}
...
function collectionReportRows(modal){
  ...
  var first=value('first'),second=value('second'),search=value('search').toLowerCase(),dateFrom=value('dateFrom'),dateTo=value('dateTo'),alertFilter=value('alertFilter');
  if(contacts){rows=rows.filter(function(row){if(first&&first!=='All contact types'&&row.contactType!==first)return false;if(second&&second!=='All states'&&row.businessState!==second)return false;if(search&&[row.contactType,row.organizationName,row.contactName,row.primaryEmail,row.officePhone,row.businessCity,row.businessState].join(' ').toLowerCase().indexOf(search)<0)return false;return true})}
  else{rows=rows.filter(function(row){if(first&&first!=='All agencies'&&row.agency!==first)return false;if(second&&second!=='All statuses'&&row.status!==second)return false;if(dateFrom&&String(row.sentDate)<dateFrom)return false;if(dateTo&&String(row.sentDate)>dateTo)return false;if(alertFilter==='Open alerts'&&row.alertsCount<1)return false;if(alertFilter==='No alert set'&&row.alertsCount>0)return false;if(search&&[row.accountId,row.customer,row.region,row.agency,row.status].join(' ').toLowerCase().indexOf(search)<0)return false;return true})}
  return rows
}
function updateCollectionsReportSummary(modal){var rows=collectionReportRows(modal);var contacts=modal.getAttribute('data-collection-report-view')==='contacts';var count=modal.querySelector('[data-collection-report-count]');var totals=modal.querySelector('[data-collection-report-totals]');if(count)count.textContent=rows.length+' '+(contacts?'contacts':'accounts');if(totals)totals.textContent=contacts?'Reusable business directory':moneyLabel(rows.reduce(function(sum,row){return sum+moneyNumber(row.amountOutstanding)},0))+' outstanding | '+moneyLabel(rows.reduce(function(sum,row){return sum+moneyNumber(row.amountCollected)},0))+' collected | '+moneyLabel(rows.reduce(function(sum,row){return sum+moneyNumber(row.amountWeReceive)},0))+' expected'}
function setQuickCollectionsReport(modal,type){var contacts=modal.getAttribute('data-collection-report-view')==='contacts';modal.querySelectorAll('input').forEach(function(field){field.value=''});modal.querySelectorAll('select').forEach(function(field){field.selectedIndex=0});if(type==='current'){if(contacts){modal.querySelector('[data-collection-report-filter="first"]').value=contactFilters.type;modal.querySelector('[data-collection-report-filter="second"]').value=contactFilters.state;modal.querySelector('[data-collection-report-filter="search"]').value=contactFilters.search}else{modal.querySelector('[data-collection-report-filter="first"]').value=collectionFilters.agency;modal.querySelector('[data-collection-report-filter="second"]').value=collectionFilters.status;modal.querySelector('[data-collection-report-filter="search"]').value=collectionFilters.search}}if(contacts&&type==='attorneys')modal.querySelector('[data-collection-report-filter="first"]').value='Attorney';if(contacts&&type==='agencies')modal.querySelector('[data-collection-report-filter="first"]').value='Collection Agency';if(!contacts&&type==='open')modal.querySelector('[data-collection-report-filter="second"]').value='Open';if(!contacts&&type==='partial')modal.querySelector('[data-collection-report-filter="second"]').value='Partially Collected';if(!contacts&&type==='received')modal.querySelector('[data-collection-report-filter="second"]').value='Received';if(!contacts&&type==='alerts')modal.querySelector('[data-collection-report-filter="alertFilter"]').value='Open alerts';updateCollectionsReportSummary(modal)}
function collectionReportTable(rows,contacts){return contacts?[['Contact Type','Organization','Contact Name','Job Title','Department','Primary Email','Secondary Email','Office Phone','Extension','Mobile Phone','Fax','Website','Preferred Contact','Business Address','Mailing Address','Notes']].concat(rows.map(function(row){return [row.contactType,row.organizationName,row.contactName,row.jobTitle,row.department,row.primaryEmail,row.secondaryEmail,row.officePhone,row.phoneExtension,row.mobilePhone,row.fax,row.website,row.preferredContactMethod,formatBusinessAddress(row,false),formatBusinessAddress(row,true),row.notes]})):[['Account / Job','Customer','Region','Collection Agency','Date Sent','Amount Outstanding','Amount Collected','Amount We Will Receive','Date Received','Status']].concat(rows.map(function(row){return [row.accountId,row.customer,row.region,row.agency,row.sentDate,moneyNumber(row.amountOutstanding),moneyNumber(row.amountCollected),moneyNumber(row.amountWeReceive),row.dateReceived,row.status]}))}
function openCollectionsReportsModal(){
  closeLiensReportsModal();var contacts=collectionView==='contacts';var modal=document.createElement('div');modal.className='citadel-modal-backdrop';modal.setAttribute('data-collection-report-view',contacts?'contacts':'accounts');
  var quick=contacts?'<button data-collection-quick="all">All Contacts</button><button data-collection-quick="attorneys">Attorneys</button><button data-collection-quick="agencies">Collection Agencies</button><button data-collection-quick="current">Current View</button>':'<button data-collection-quick="alerts">Open Alerts</button><button data-collection-quick="open">Open Accounts</button><button data-collection-quick="partial">Partially Collected</button><button data-collection-quick="received">Received</button><button data-collection-quick="current">Current View</button>';
  var filters=contacts?'<label>Contact Type<select data-collection-report-filter="first">'+renderSelectOptions('All contact types',getCollectionContactTypes(),'All contact types')+'</select></label><label>State<select data-collection-report-filter="second">'+renderSelectOptions('All states',getCollectionContactStates(),'All states')+'</select></label><label>Search Text<input data-collection-report-filter="search" placeholder="Organization, contact, phone, email"></label>':'<label>Agency<select data-collection-report-filter="first">'+renderSelectOptions('All agencies',getCollectionAgencies(),'All agencies')+'</select></label><label>Status<select data-collection-report-filter="second">'+renderSelectOptions('All statuses',['Open','Partially Collected','Received'],'All statuses')+'</select></label><label>Date Sent From<input data-collection-report-filter="dateFrom" type="date"></label><label>Date Sent To<input data-collection-report-filter="dateTo" type="date"></label><label>Alert Filter<select data-collection-report-filter="alertFilter"><option>All alerts</option><option>Open alerts</option><option>No alert set</option></select></label><label>Search Text<input data-collection-report-filter="search" placeholder="Account, customer, agency, region"></label>';
  modal.innerHTML='<section class="citadel-report-modal" role="dialog" aria-modal="true" aria-label="Collections Reports"><div class="modal-head"><div><h3>Collections Reports</h3><p>'+(contacts?'Export the reusable business-contact directory.':'Report on jobs sent to collections and recovery progress.')+'</p></div><button type="button" data-close-modal aria-label="Close">x</button></div><div class="report-callout">'+(contacts?'Contact exports include complete business and mailing information.':'Collection exports include agency, dates, outstanding, collected, expected receipt, and received date.')+'</div><section class="report-band"><div class="report-band-head"><strong>Quick Reports</strong><span>Common exports</span></div><div class="quick-report-grid">'+quick+'</div></section><section class="report-custom"><div class="report-band-head"><strong>Custom Report</strong><span>Filters and export format</span></div><div class="report-form-grid">'+filters+'</div></section><div class="report-summary-row"><div class="report-parameters"><strong>Report Parameters</strong><span>View: <b>'+(contacts?'Business Contacts':'Collection Accounts')+'</b></span><span>Generated: <b>'+escapeHtml(new Date().toLocaleString())+'</b></span></div><div class="report-total"><strong data-collection-report-count>0</strong><span>Matching records</span><em data-collection-report-totals></em></div></div><div class="modal-actions"><button type="button" data-collection-report-reset>Reset</button><span></span><select data-collection-report-format><option value="csv">CSV</option><option value="excel">Excel</option><option value="pdf">PDF</option></select><button type="button" data-close-modal>Cancel</button><button type="button" data-collection-report-export>Export</button></div></section>';
  modal.addEventListener('click',function(event){var quickButton=event.target.closest('[data-collection-quick]');if(quickButton){setQuickCollectionsReport(modal,quickButton.getAttribute('data-collection-quick'));return}if(event.target.closest('[data-collection-report-reset]')){setQuickCollectionsReport(modal,'all');return}if(event.target.closest('[data-collection-report-export]')){var rows=collectionReportRows(modal);var format=modal.querySelector('[data-collection-report-format]').value;reportDownloadTable(collectionReportTable(rows,contacts),format,contacts?'citadel-business-contacts':'citadel-collections-report');return}if(event.target===modal||event.target.closest('[data-close-modal]'))closeLiensReportsModal()});
  modal.addEventListener('input',function(){updateCollectionsReportSummary(modal)});modal.addEventListener('change',function(){updateCollectionsReportSummary(modal)});document.body.appendChild(modal);updateCollectionsReportSummary(modal)
}
...
var pageDetails={"data-connections":{section:"Source management",focus:"Monitor integrations, feed status, sync health, credential expiration, and the quality checks that keep the platform trustworthy.",metrics:[["Sources","24","21 healthy"],["Failed Syncs","3","needs review"],["Freshness","12m","median delay"],["Mappings","148","5 pending"]],queue:[["Broken feeds","3"],["Pending maps","5"],["Warnings","12"]]},inbox:{section:"Message triage",focus:"Centralize incoming requests, escalations, shared notes, and system-generated messages so nothing important sits outside the workflow.",metrics:[["Unread","37","9 priority"],["Assigned","22","active"],["Escalations","6","legal included"],["Avg Response","1.8h","within target"]],queue:[["Urgent","9"],["Needs owner","7"],["Waiting","14"]]},tasks:{section:"Execution queue",focus:"Coordinate task ownership, due dates, blockers, dependencies, and completion status across operational teams.",metrics:[["Open Tasks","126","31 due today"],["Blocked","12","owner needed"],["Completed","48","today"],["On Time","91%","rolling 7 days"]],queue:[["Overdue","11"],["Due today","31"],["Unassigned","6"]]},legal:{section:"Risk workflow",focus:"Track matters, exceptions, approval gates, document requests, and legal review items with clear ownership and next actions.",metrics:[["Open Matters","18","4 urgent"],["Approvals","9","pending"],["Docs Needed","15","from field"],["Risk Flags","7","watchlist"]],queue:[["New matters","4"],["Review needed","9"],["Expired docs","3"]]},reviews:{section:"Quality control",focus:"Manage operational reviews, audit outcomes, exception scoring, quality checks, and approval history in one place.",metrics:[["Reviews","64","this cycle"],["Pass Rate","88%","up 3%"],["Rechecks","10","scheduled"],["Findings","21","open"]],queue:[["Awaiting review","16"],["Recheck","10"],["Approved","38"]]},pricing:{section:"Rate intelligence",focus:"Compare pricing signals, margin movement, exceptions, contract terms, and recommended adjustments without crowding the rest of the app.",metrics:[["Active Rates","312","across regions"],["Exceptions","17","needs approval"],["Margin","22.8%","weighted"],["Change Queue","26","pending"]],queue:[["Approve","17"],["Model updates","6"],["Expiring","12"]]},"fleet-vehicles":{section:"Fleet management",focus:"Maintain vehicle records, registration status, utilization, service windows, and compliance exceptions for the active fleet.",metrics:[["Vehicles","284","247 active"],["Service Due","18","next 14 days"],["Utilization","73%","fleet average"],["Offline","11","review"]],queue:[["Service due","18"],["Missing docs","7"],["Inactive","11"]]},"fleet-drivers":{section:"Fleet management",focus:"Track driver status, onboarding, documents, credential expiration, region assignments, and performance exceptions.",metrics:[["Drivers","412","389 active"],["Expiring Docs","23","next 30 days"],["Onboarding","31","in progress"],["Flagged","8","review"]],queue:[["Docs expiring","23"],["Onboarding","31"],["Flagged","8"]]},"region-health":{section:"Market signals",focus:"Evaluate regional capacity, backlog, compliance pressure, contractor coverage, and revenue risk by market.",metrics:[["Regions","18","tracked"],["Healthy","14","green"],["Watchlist","4","needs action"],["Coverage","92%","target 95%"]],queue:[["Watchlist","4"],["Capacity gaps","6"],["Escalations","3"]]},contractors:{section:"Partner network",focus:"Manage contractor profiles, documents, assignments, coverage zones, quality scores, and payment readiness.",metrics:[["Contractors","156","139 active"],["Pending Docs","29","follow up"],["Coverage Gaps","6","regions"],["Score Avg","4.6","last 90 days"]],queue:[["Pending docs","29"],["Low score","5"],["Needs assignment","12"]]},collections:{section:"Recovery workflow",focus:"Prioritize collection queues, outreach status, payment plans, disputes, risk levels, and recovery progress across accounts.",metrics:[["Accounts","238","open"],["Recovery","$412K","pipeline"],["Disputes","16","active"],["Plans","38","current"]],queue:[["High risk","22"],["Disputes","16"],["Payment plans","38"]]}};
...
var STANDARD_FILTERS={
  ...
  "fleet-drivers":[["Status","All statuses",["Active","Onboarding","Flagged","Inactive"]],["Credential","All credentials",["Expiring Soon","Expired","Missing","Current"]],["Region","All regions",["Wisconsin","St. Louis","Indianapolis","Chicago"]],["Sort","Expiring soon",["Expiring soon","Flagged first","Driver name"]]],
  "region-health":[["Region","All regions",["Wisconsin","St. Louis","Indianapolis","Chicago","Santa Fe"]],["Health","All health",["Healthy","Watchlist","Critical","No Coverage"]],["Coverage","All coverage",["Covered","Gap","Low Capacity","Escalated"]],["Sort","Needs attention",["Needs attention","Coverage low","Region name"]]],
  collections:[["Status","All statuses",["Open","Dispute","Payment Plan","Escalated"]],["Stage","All stages",["Monitor","Review","High","Critical"]],["Region","All regions",["Wisconsin","St. Louis","Indianapolis","Chicago"]],["Sort","Highest balance",["Highest balance","Oldest first","Newest update"]]]
};
...
function reportDownloadTable(table,format,filenameBase){format=(format||'csv').toLowerCase();if(format==='excel'){var html='<!doctype html><html><head><meta charset="UTF-8"></head><body><table>'+table.map(function(row){return '<tr>'+row.map(function(cell){return '<td>'+escapeHtml(cell==null?'':cell)+'</td>'}).join('')+'</tr>'}).join('')+'</table></body></html>';downloadReportBlob('\uFEFF'+html,'application/vnd.ms-excel;charset=utf-8',filenameBase+'.xls');return}var csv='\uFEFF'+table.map(function(row){return row.map(function(cell){return '"'+String(cell==null?'':cell).replace(/"/g,'""')+'"'}).join(',')}).join('\n');downloadReportBlob(csv,'text/csv;charset=utf-8',filenameBase+'.csv')}
function openWorkspaceReportsModal(title,description,count,onExport){var modal=document.createElement('div');modal.className='citadel-modal-backdrop';modal.innerHTML='<section class="citadel-report-modal" role="dialog" aria-modal="true" aria-label="'+escapeHtml(title)+'"><div class="modal-head"><div><h3>'+escapeHtml(title)+'</h3><p>'+escapeHtml(description)+'</p></div><button type="button" data-close-modal aria-label="Close">x</button></div><div class="report-callout">Choose the report format, then export the current filtered view.</div><div class="report-band"><div class="report-band-head"><strong>Quick Reports</strong><span>Common exports</span></div><div class="quick-report-grid"><button type="button" data-simple-report="current">Current View</button></div></div><div class="report-summary-row"><div class="report-parameters"><strong>Report Parameters</strong><span>Rows included: <b>'+escapeHtml(count)+'</b></span><span>Source: <b>Current filters</b></span></div><div class="report-total"><strong>'+escapeHtml(count)+' records</strong><span>Ready to export</span><em>CSV or Excel</em></div></div><div class="modal-actions"><button type="button" data-close-modal>Cancel</button><span></span><select data-simple-report-format><option value="csv">CSV</option><option value="excel">Excel</option></select><button type="button" data-simple-report-export>Export</button></div></section>';modal.addEventListener('click',function(event){if(event.target===modal||event.target.closest('[data-close-modal]')){closeLiensReportsModal();return}if(event.target.closest('[data-simple-report-export]')||event.target.closest('[data-simple-report]')){var field=modal.querySelector('[data-simple-report-format]');onExport(field?field.value:'csv')}});document.body.appendChild(modal)}
function openStandardReportsModal(){openWorkspaceReportsModal(getPageLabel(activePage)+' Reports','This page report builder is ready for source data wiring.',0,function(){})}
